...
IplImage* cvLoadImageFromFile(const char* filename, int flags)
{
	...
	fseek(p, bmpinfohead.startPosition, SEEK_SET);//可以跳过调色板
...
int cvSaveImage(const char* filename, const CvArr* image)
{
    ...
    int step = src->width * src->nChannels;//windows位图step必须是4的倍数
...
/*
 * pdst-目标bmp图地址 imgData-图像源地址，width，height宽高，widthstep 字节对齐源图每行字节数（8字节），channel通道数
 */
